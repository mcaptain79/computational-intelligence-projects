...
#function below is implementation of sigmoid function and adding it to numpy library
def sigmoid(x):
    if x > 0:
        return x
    return 0
...
sigmoid_prime = np.frompyfunc(sigmoid,1,1)
    ...
    for i in range(number_of_epoches):
        ...
        for j in range(batch_size):
            for k in range(j*6000,j*6000+6000):
                ...
                z3,a3 = level3_AZ(a2)
                res = np.dot(weights_level3,a3)+bias_level3
                weights_level3_gradient = weights_level3_gradient+np.dot(2*sigmoid_prime(res)*(sigmoid(res)-y_calculator(k)),a3.transpose())
...
print(test(testData, testLabels))
