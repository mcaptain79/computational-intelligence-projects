    ...
    for i in range(number_of_epoches):
        ...
        for j in range(batch_size):
            for k in range(j*6000,j*6000+6000):
                ...
                z3,a3 = level3_AZ(a2)
                res = np.dot(weights_level3,a3)+bias_level3
                weights_level3_gradient = weights_level3_gradient+np.dot(2*activation_function_prime(res)*(activation_function(res)-y_calculator(k)),a3.transpose())
...
def test(testData,testLabels):
    ...
    for i in range(len(testData)):
        ...
        if find_max_element(res, 10, 1)[0] == trainLabels[i]:
            counter += 1
    return counter/100
print('accuracy:',test(testData, testLabels))
