    ...
    for i in range(number_of_epoches):
        ...
        for j in range(batch_size):
            for k in range(j*6000,j*6000+6000):
                ...
                weights_level2_gradient = weights_level2_gradient + np.dot(middle21,a2.transpose())
                #middle11 = np.dot(weights_level3.transpose(),calc)
                #middle12 = np.dot(weights_level2,middle11)
                #weights_level1_gradient = weights_level1_gradient + np.dot(middle12,trainData[k].reshape(1,784))
                #bias_level3_gradient = bias_level3_gradient+calc
                #bias_level2_gradient = bias_level2_gradient+np.dot(weights_level3.transpose(),calc)
                #middle31 = np.dot(weights_level3.transpose(),calc)
                #bias_level1_gradient = bias_level1_gradient + np.dot(weights_level2,middle31)
            ...
            #bias_level3 = bias_level3 - (learning_rate*(bias_level3_gradient/batch_size))
            #bias_level2 = bias_level2 - (learning_rate*(bias_level2_gradient/batch_size))
            #bias_level1 = bias_level1 - (learning_rate*(bias_level1_gradient/batch_size))
"""
...
def test(testData,testLabels):
    ...
    for i in range(len(testData)):
        z2 = np.dot(weights_level1,testData[i].reshape(784,1))+bias_level1
        a2 = activation_function(z2)
        z3 = np.dot(weights_level2,a2)+bias_level2
        a3 = activation_function(z3)
        res = np.dot(weights_level3,a3)
