...
class Player():
    ...
    def __init__(self, mode = 'helicopter', control=False):
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        # velocity example: 7
        max_velocity = 10
        myVelocity = velocity/max_velocity
        if len(box_lists) > 2:
            distanceToOne = (agent_position[0] - box_lists[0].x)/1280
            distanceToTwo = (agent_position[0] - box_lists[1].x)/1280
            distanceToThree = (agent_position[0] - box_lists[2].x)/1280
            distanceToOne2 = (agent_position[1] - box_lists[0].gap_mid)/720
            distanceToTwo2 = (agent_position[1] - box_lists[1].gap_mid)/720
            distanceToThree2 = (agent_position[1] - box_lists[2].gap_mid)/720
        else:
            distanceToOne = 0.5
            distanceToTwo = 0.5
            distanceToThree = 0.5
            distanceToOne2 = 0.5
            distanceToTwo2 = 0.5
            distanceToThree2 = 0.5
        parameters = np.array([[myVelocity],[distanceToOne],[distanceToTwo],[distanceToThree],[distanceToOne2],[distanceToTwo2],[distanceToThree2]])
        tavak = self.nn.forward(parameters)
        direction = -1
        if tavak < 0.5:
            return direction            
        return direction*-1
